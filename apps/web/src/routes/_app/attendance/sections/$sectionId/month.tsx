/** A class's month: every pupil, every day, and what the month adds up to. */
...
export const Route = createFileRoute('/_app/attendance/sections/$sectionId/month')({ component: Page, validateSearch: searchSchema })
...
function Page() {
  ...
  const monthQuery = useQuery({
    queryKey: qk.attendanceSectionMonth(schoolId, sectionId, month),
    queryFn: () => api.attendance.sectionMonth(schoolId, sectionId, month),
  })

  const startExport = useMutation({
    mutationFn: (format: FeeFileFormat) => api.attendance.exportSectionMonth(schoolId, sectionId, month, { format }),
    onSuccess: (job) => {
  ...
  const header = (
    <PageHeader
      crumbs={[{ label: 'Attendance', to: '/attendance', icon: <ClipboardCheck /> }, { label: title }]}
      actions={data && allows(data.allowedActions, 'attendance.export')
  ...
  if (monthQuery.isError) {
    return (
      <>
        {header}
        <EmptyState icon={<ClipboardCheck />} title="This register is not available" description={describeError(monthQuery.error)} />
      </>
    )
  }
  ...
  const rows: RegisterGridRow[] = (data?.rows ?? []).map((row) => ({
    ...
    cells: row.marks.map((mark) => ({ on: mark.enrolled, mark: mark.mark })),
    summary: row.summary,
  }))
    ...
    <>
      ...
      <Toolbar>
        <MonthChip
          month={month}
          max={currentMonth()}
          onChange={(value) => void navigate({ to: '/attendance/sections/$sectionId/month', params: { sectionId }, search: { month: value }, replace: true })}
        />
      </Toolbar>
      {exportFile.status}
      {!data ? (
        ...
        <EmptyState icon={<ClipboardCheck />} title="Nobody to show" description="No pupil was on this roster in this month." />
      ) : (
        <RegisterGrid days={data.days} rows={rows} leadLabel="Roll" />
      )}
      <div className="flex h-11 shrink-0 items-center gap-4 border-t bg-card px-3 text-[12.5px] text-muted-foreground md:px-4">
        <span>{rows.length} {rows.length === 1 ? 'pupil' : 'pupils'}</span>
      </div>
    </>
  )
}
