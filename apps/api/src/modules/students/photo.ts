...
const PATH = '/api/schools/:schoolId/students/:studentId/photo'
...
function studentId(request: FastifyRequest): string {
  const params = request.params as { studentId?: string }
  return assertUuidParam(params.studentId ?? '')
}
...
function photoKey(schoolId: string, id: string): string {
  return `photos/${schoolId}/student/${id}-${randomBytes(16).toString('hex')}`
}
...
/**
 * A photograph may only be held while the family has said yes to it. The
 * newest answer per guardian is the current one: at least one guardian has to
 * have given consent and none may have withdrawn it.
 */
async function photographsConsented(
  conn: TenantConnection,
  schoolId: string,
  id: string,
): Promise<boolean> {
  const rows = await conn.client.query<{ status: string }>(
    `SELECT DISTINCT ON (guardian_id) status
       FROM guardian_consents
      WHERE school_id = $1 AND student_id = $2 AND purpose = 'photographs'
      ORDER BY guardian_id, recorded_at DESC, id DESC`,
    [schoolId, id],
  )
  const answers = rows.rows.map((row) => row.status)
  return answers.includes('given') && !answers.includes('withdrawn')
}

/**
 * Student photographs (Task 16). The bytes live in the private document store
 * and are only ever served by the streaming route below, which decides the
 * record again for every request. Nothing here trusts what an upload claims
 * to be: the type comes from the first bytes of the file and the picture is
 * rewritten without its description blocks before it is stored.
 */
export function registerStudentPhotoRoutes(app: FastifyInstance, deps: ModuleDependencies): void {
  ...
  void app.register(async (scope) => {
      ...
      async (request, reply) => {
        ...
        try {
          previous = await withTenantTransaction(deps.pools.runtime, context, async (conn) => {
            ...
            await authorizeResource(conn, context, 'students.update_basic', 'student', id)
            const row = await loadPhotoRow(conn, context.schoolId, id)
            if (!(await photographsConsented(conn, context.schoolId, id))) {
              // Nothing about the request is wrong; the family has not agreed.
            ...
            await writeAudit(conn, context, {
              ...
              targetType: 'student',
              targetId: id,
              summary: 'Set the student photograph.',
    ...
    scope.delete(PATH, { preHandler: requireMembership(deps) }, async (request, reply) => {
      ...
      const previous = await withTenantTransaction(deps.pools.runtime, context, async (conn) => {
        ...
        await writeAudit(conn, context, {
          ...
          targetType: 'student',
          targetId: id,
          summary: 'Removed the student photograph.',
     ...
     * this exact record. A student the caller may not read answers exactly
     * like one that does not exist. Looking at a photograph is not an event a
     * school needs recorded, so no audit row is written.
     */
    scope.get(PATH, { preHandler: requireMembership(deps) }, async (request, reply) => {
      ...
      const photo = await withTenantTransaction(deps.pools.runtime, context, async (conn) => {
        await authorizeSchoolAction(conn, context, 'students.read_basic')
        const decision = await decideResource(conn, context, 'students.read_basic', 'student', id)
        if (!decision.allowed) {
