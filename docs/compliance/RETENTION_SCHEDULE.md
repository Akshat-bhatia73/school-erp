...
| **Your child's exam results**: every mark or status (absent, medical leave, exempt) per exam and component, every change with who made it and the kind of reason, when the school published each exam's results, and the grades for work education, art education, health and physical education and discipline | Permanently, as part of your child's academic record, like the admission register. A school issues a mark statement or a transfer certificate years later from the same record | Nothing. A mark is never edited or deleted: a change is a new entry and the old one stays. The words someone typed as the reason are a note on the audit entry and can be redacted |
...
| **Conversations with the school assistant**: each question, each answer and the records looked up to answer it, kept sealed so only the person who asked can read them | 30 days after each message was written | Deleted every night once 30 days have passed. Deleting a conversation removes it at once, and anonymising a pupil removes their own conversations at once |
| **Assistant question counts**: who asked how many questions on which day, and what it cost, with no words | 13 months, for the limits and the bill | Deleted every night once the period is over |
...
| **Logins and credentials**: password, second factor, backup codes (a pupil's own login in Class 9 to 12 included; it ends when the pupil leaves) | While the person holds an active membership at the school | Sessions end the moment the last membership is removed. The credentials themselves are deleted 30 days later. The identity's id and name are kept so old audit entries still say who did something |
