...
**Two routes to the model.** The deployment chooses one with `ASSISTANT_PROVIDER`. **Google AI Studio directly** (`google`, chosen by the product owner on 25 September 2026 for the test environment): the Gemini API with the school's own key. On a free-tier key Google may use what it is sent to improve its products and people may review it, so a free key is for test data only; before any real child's data, the key's Google Cloud project must have billing on, where Google does not use prompts or answers for that and keeps them only for a limited time to detect abuse (`[confirm the period in Google's current Gemini API terms]`), and the privacy notice must name this route. **Vercel AI Gateway to Google Vertex AI** (`gateway`) is described next.

**What leaves the country.** With the gateway, questions, answers and the records used to answer them go to Google Vertex AI through Vercel AI Gateway. Every request is sent with zero data retention: the gateway keeps nothing after the request, and it routes only to Google under the retention agreement Vercel holds, failing the request rather than using any other route. Where Google processes the request is `[to confirm: the gateway does not let us choose a Vertex AI region; record the region Vercel states for Google Vertex AI]`. The transfer is allowed under section 16 of the Act because no country has been restricted. Per-request zero data retention needs the Vercel Pro plan, so **the Pro plan comes before any real child's data goes through the assistant**.
