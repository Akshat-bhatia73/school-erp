...
| Google AI Studio / Gemini API (Google LLC), called directly | Runs the language model that answers the assistant's questions when `ASSISTANT_PROVIDER=google` (Task 24). | School records in transit, as above, including children's. On a free-tier key Google may use them to improve its products and people may review them: test data only. On a key whose Google Cloud project has billing on, not used for that; kept for a limited time for abuse monitoring `[confirm the period in Google's current terms]` | `[the Gemini API does not let us pick a processing region; record what Google states]` | 25 Sep 2026 |
