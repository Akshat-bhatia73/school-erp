...
### Task 11: Hosting and the data protection assessment

Delivered 17 September 2026 (PRs 15 and 16): the API as a Vercel Function beside the site, Resend, Vercel Blob, Sentry, the held-text-message route for test builds, and [docs/compliance/DATA_PROTECTION.md](compliance/DATA_PROTECTION.md), whose findings F1 to F19 define Tasks 12 to 14.

### Task 12: Data lifecycle and consent

Owner: one migration owner for `packages/db`, one contracts owner for `packages/contracts` and `packages/authz`, backend agents for `apps/api`, one frontend agent for `apps/web`, one docs agent. Added on 18 September 2026 to close findings F1, F2, F3, F4, F5, F9, F13, F14 and F15 of the data protection assessment. Depends on Task 11. The access-control model does not change: every new route goes through `protectedRoute`, every list is narrowed by `planPredicate`, every write locks the school and writes exactly one audit row.

**Decisions.**

- Retention periods are constants in `@erp/contracts` (`RETENTION`): student sensitive fields 3 years after leaving, staff private and pay fields 8 years after leaving, login credentials 30 days after the last membership ends, invitations and outbox rows 90 days after they become terminal, import previews 24 hours, sessions, one-time codes and throttle rows until expiry. Anonymisation is a decision by the school, taken through a permission-gated route that refuses while the period is still running. The sweeper deletes only transient copies; it never touches a person's record.
- One migration, `0009_data_lifecycle.sql`, owns every schema change below. The only data anywhere today is fixture and seed data, so `students.apaar_id` and the three `photo_url` columns are dropped in this migration rather than staged over two releases.
- Sensitive text is sealed with AES-256-GCM in the API under `DATA_ENCRYPTION_KEY` (base64 of 32 bytes, required at startup, production refuses the example value). Ciphertext is stored as `v1.<iv>.<tag>.<ciphertext>` in base64url so a later key or algorithm can coexist. The key never reaches the database.
- A daily sweep is `GET /api/maintenance/sweep`, present only when `CRON_SECRET` is configured and accepted only with `Authorization: Bearer <CRON_SECRET>` (the header Vercel Cron sends). `vercel.json` schedules it once a day. The cross-school work runs in `SECURITY DEFINER` functions owned by a new `erp_maintenance` NOLOGIN role that has its own permissive policies on the transient tables, following the `erp_identity_reader` pattern; no role gains `BYPASSRLS`.

**Schema (`packages/db/migrations/0009_data_lifecycle.sql`, mirrored in `packages/db/src/schema.ts`).**

1. `guardian_consents`: `id`, `school_id`, `student_id`, `guardian_id`, `purpose` (CHECK in `education_records`, `health_information`, `photographs`, `communication`, `third_party_services`), `status` (`given` | `withdrawn`), `method` (`in_person` | `signed_form` | `portal`), `recorded_by_membership_id` NOT NULL, `evidence_reference` text, `recorded_at` timestamptz default now(), `created_at`. Composite foreign keys to students, guardians and memberships. Index `(school_id, student_id, guardian_id, purpose, recorded_at DESC)`. Tenant RLS, forced. A trigger refuses UPDATE and DELETE, as `audit_events` does: every row is an event and the current state of a purpose is its newest row. `GRANT SELECT, INSERT` to `erp_runtime`.
2. `audit_event_notes`: `id`, `school_id`, `audit_event_id` NOT NULL UNIQUE references `audit_events(id)`, `note` text NOT NULL, `redacted_at`, `redacted_by_membership_id`, `created_at`. Tenant RLS, forced. No append-only trigger. `GRANT SELECT, INSERT, UPDATE` to `erp_runtime`. `audit_events.safe_changes` stays structural: no free text from a request body may be written into it.
3. `students`: add `anonymised_at timestamptz`, `apaar_last4 text`, `apaar_ciphertext text`; drop `apaar_id` and `photo_url`. `guardians`: add `anonymised_at`; drop `photo_url`. `staff`: add `anonymised_at`; drop `photo_url`.
4. `REVOKE DELETE ON students, guardians, staff, student_documents FROM erp_runtime`. No API code deletes from these today; removal is a status change plus anonymisation.
5. Role `erp_maintenance` (NOLOGIN, NOBYPASSRLS) with `USAGE` on the schema, policies `FOR ALL TO erp_maintenance USING (true) WITH CHECK (true)` on `student_import_previews`, `export_jobs`, `delivery_outbox`, `school_invitations`, and `FOR SELECT` on `school_memberships`; table grants to match. Three functions owned by it, `SECURITY DEFINER`, `search_path = pg_catalog, public`, each returning the counts it deleted or updated as a table of `(item text, count integer)`:
   - `sweep_tenant_transients()`: delete `student_import_previews` past `expires_at` or committed more than a day ago; delete `export_jobs` a day past `expires_at`; delete `delivery_outbox` rows delivered or failed more than 90 days ago; on `school_invitations` that are no longer pending, set `identifier_normalized` to `''` (the terminal-revival trigger must allow this: it refuses only a change of status), and delete those terminal more than 90 days. Execute granted to `erp_runtime` only.
   - `sweep_auth_transients()`: delete `auth_verification`, `auth_session`, `auth_throttle` and `held_sms` rows past `expires_at`. Execute granted to `erp_auth` only.
   - `sweep_orphaned_credentials(grace interval)`: for every `auth_user` with no membership in status `active` or `suspended` whose newest membership row changed longer ago than `grace` (or who has no membership row and was created longer ago than `grace`), delete their `auth_session`, `auth_account` and `auth_two_factor` rows and set `email` to `removed+<id>@invalid.local`, `phone_number` NULL, both verified flags false, `two_factor_enabled` false, `image` NULL. `name` and `id` stay so audit rows keep their attribution. Execute granted to `erp_auth` only.
   `packages/db/tests` prove: the runtime cannot DELETE from the four tables; consents cannot be updated or deleted; the functions cannot be executed by the wrong role; a sweep removes an expired preview from School A and School B in one call under no tenant context; the invitation blanking succeeds on a terminal row.

**Contracts (`@erp/contracts`, `@erp/authz`).**

- New permissions: `students.read_consents` (student; school, own_children), `students.manage_consents` (student; school, own_children; privileged at school scope only, so a parent signed in with a phone code can answer for their own child), `students.anonymise` (student; school; privileged), `staff.anonymise` (staff; school; privileged), `audit.redact_notes` (audit_event; school; privileged). Owner: all five. Principal: all but `audit.redact_notes`. Admin: the two consent permissions. Parent: the two consent permissions at `own_children`. Teacher, accountant, student: none. Regenerate `PERMISSION_MATRIX.generated.md`; update `endpoints.ts` and the policy contract tests.
- `CONSENT_PURPOSES`, `ConsentPurpose`, `ConsentMethod`, `ConsentStatus`. `ConsentRecord` = `{ id, studentId, guardianId, guardianDisplayName, purpose, status, method, evidenceReference?, recordedAt, recordedBy: 'office' | 'guardian' }`. `ConsentList` = `{ items: ConsentRecord[], allowedActions }`, newest row per `(guardian, purpose)` only. `RecordConsentRequest` = `{ guardianId, purpose, status, method, evidenceReference? (max 200) }`, strict.
- `StudentsAdmitRequest` gains `consents?: Array<{ guardianIndex (int, index into guardians), purpose, method, evidenceReference? }>` max 25; a `guardianIndex` outside the array is `INVALID_REQUEST`. Import rows are unchanged.
- `StudentSensitive.apaarId` is replaced by `apaarMasked` matching `^XXXX-XXXX-\d{4}$`. `StudentApaarReveal` = `{ apaarId }`. Write requests keep `apaarId` as input, trimmed, 4 to 100 characters.
- `AnonymiseRequest` = `{ expectedVersion, reason }` for students and staff. `UnlinkGuardianRequest` = `{ expectedVersion, reason }` where `expectedVersion` is the student's version. `AuditEventSummary` gains `note?` (max 1000). `RedactAuditNoteRequest` = `{ reason }`. `RETENTION` as above.

**API (`apps/api`).** Routes, each behind `protectedRoute`:

- Audit notes. `recordAuditEvent` accepts an optional `note`; when present it inserts the `audit_event_notes` row in the same transaction and the note never enters `safe_changes`. Every call site that today writes `reason: body.reason` into `safeChanges` (pay change, assignment, move, leaving, role change, suspension and removal, recovery, ownership transfer) passes it as `note` instead. `GET /audit-events` returns `note` on rows whose note is not redacted, through a LEFT JOIN under the same plan predicate. `POST /audit-events/:eventId/note/redact` (`audit.redact_notes`) sets `redacted_at` and the redacting membership, and writes an audit row with no note.
- APAAR. `apps/api/src/modules/shared/crypto.ts`: `seal(plain, key)` and `open(ciphertext, key)` with AES-256-GCM, plus `maskApaar(last4)`. Writes store `apaar_ciphertext` and `apaar_last4`. The sensitive block shows `apaarMasked`. `GET /students/:studentId/apaar` (`students.read_sensitive`) decides the record, returns the full value and writes an audit row `students.read_sensitive` on the student with summary "Revealed the full APAAR id." Search never matches APAAR.
- Roster over-fetch. `getStudent`, `listStudents`, `searchStudents` and `countStudents` select the sensitive and medical columns only when the caller asked for them: the detail route decides `students.read_sensitive` and `students.read_medical` first and passes `{ sensitive, medical }` into the read; the roster, search and count never select them.
- Export. `POST /students/export` inserts the job as `queued`, like the staff and audit exports.
- Consents. `GET /students/:studentId/consents` (`students.read_consents`) and `POST /students/:studentId/consents` (`students.manage_consents`), both deciding the student record under the route permission; the guardian must be linked to that student in this school. A parent recording through the portal is stored with `method = 'portal'` regardless of what was sent, and `recordedBy` is `'guardian'` when the actor's membership is linked to that guardian. `recordConsents(conn, context, studentId, entries)` in `modules/students/consents.ts` is the one insertion path and is what admission calls for the `consents` array. One audit row per request, with the purpose and status list in `safeChanges` and no evidence text.
- Anonymisation. `POST /students/:studentId/anonymise` (`students.anonymise`) refuses unless status is `left` or `alumni` and the newest `left_on` is at least `RETENTION` years ago (a new `NOT_ALLOWED_YET` error code, 409, also used for the unlink refusal below; the test back-dates the row to pass the period). It clears `date_of_birth`, `gender`, `blood_group`, `category`, `religion`, `mother_tongue`, `nationality`, `aadhaar_last4`, `apaar_ciphertext`, `apaar_last4`, `address`, `previous_school`, `left_reason`, `medical_notes`, `house`, `basic_data`, `sensitive_data`, `medical_data`, blanks `file_name` and `storage_key` to `''` on each `student_documents` row inside the transaction (rows stay because DELETE is revoked; the projection hides blanked rows) and removes the bytes from storage after the commit, so a rolled-back request destroys nothing, sets `anonymised_at`, bumps the version, and then anonymises every linked guardian who has no other student that is not anonymised: `first_name` becomes `Former guardian`, `last_name`, `phone`, `alt_phone`, `email`, `occupation`, `qualification`, `annual_income`, `address`, `contact_data`, `private_data` cleared, `anonymised_at` set. Register fields (`first_name`, `last_name`, `admission_number`, `status`, `admission_date`, `admission_type`, enrolments) stay. `POST /students/:studentId/guardians/:guardianId/unlink` (`students.manage_guardians`) removes the `student_guardians` row, refuses to remove the last guardian of an active student, and anonymises the guardian the same way when no link remains. `POST /staff/:staffId/anonymise` (`staff.anonymise`) refuses unless status is `resigned` or `retired` and `leaving_date` is at least 8 years ago; it clears `phone`, `email`, `address`, `date_of_birth`, `gender`, `blood_group`, `monthly_salary`, `bank_account_last4`, `pan_last4`, `qualification` and sets `anonymised_at`; `display name`, `employee_code`, `designation`, `department`, employment dates and status stay. Anonymised students and staff report `anonymised: true` in their basic block and no sensitive, medical, private or pay block. Every anonymisation writes one audit row with the note as the reason.
- Membership removal. When a `remove` commits and the identity function reports no other active membership for the person, delete every `auth_session` row of that user in the same request, through the auth pool after the tenant transaction commits. A person suspended elsewhere signs in again after a restore; that is the safe side. The credential rows go 30 days later through the sweep.
- Sweep. `apps/api/src/maintenance/`: `GET /api/maintenance/sweep` registered only when `CRON_SECRET` is set, compares the bearer token in constant time, runs the three functions (runtime pool, auth pool, auth pool with `RETENTION.credentialGraceDays`), logs one structured line with the counts, and returns them. No session, no school, no audit row. Throttle keys in `auth/phone-otp.ts` carry a SHA-256 of the phone number, never the number.
- Configuration. `DATA_ENCRYPTION_KEY` required; `CRON_SECRET` optional, 32+ characters; both in `.env.example` with development values production refuses. `ModuleDependencies` carries `config`.

Tests in `apps/api/tests`: a note never appears in `safe_changes` and does appear on the audit row for a school reader and disappears after redaction; the sensitive block carries only the mask and the reveal route returns the value and leaves an audit row; a roster SQL statement for a basic reader does not name `medical_notes` (assert through a query log on the pool or by reading `pg_stat_statements`-free means such as spying the `execute` calls in a unit test of `reads.ts`); the export job is `queued`; a parent records and withdraws a consent for their own child and is refused for another child; an office user records consent at admission; anonymisation is refused inside the period, succeeds after back-dating `left_on`, leaves the register fields, clears the rest, anonymises the orphaned guardian and keeps the shared one; the sweep removes expired rows in both schools and is refused without the bearer token and absent without `CRON_SECRET`; a removed member with no other school has no sessions left.

**Web (`apps/web`).** Admission gets a Consent step between Guardians and Class: for each guardian, the five purposes as checkboxes with the method and an optional evidence reference; the review step lists them. The student profile gets a Consent panel (records, and record or withdraw for `students.manage_consents`), shows the masked APAAR with a Reveal action for `students.read_sensitive` that fetches the value on demand and never caches it, an Unlink action on each guardian for `students.manage_guardians`, and an "Anonymise record" action in the danger area for `students.anonymise` that explains what stays and what goes. The parent home lists each child's consents with Give and Withdraw. The staff profile gets the same anonymise action for `staff.anonymise`. The audit screen shows the note under the summary and a Redact action for `audit.redact_notes`. Anonymised records show an "Anonymised" tag and hide the empty blocks.

**Docs.** `DATABASE.md`, `CONTRACTS.md`, `PROTECTED_APIS.md`, `ACCESS_MANAGEMENT.md`, `WEB_SCREENS.md`, `OPERATION_COVERAGE.md`, `RELEASE.md` (the two variables, the cron, the retention schedule as adopted) and `DATA_PROTECTION.md` (each finding marked closed with the file that closes it; the retention schedule moved from proposed to adopted).

Exit check: a parent's consent is recorded and withdrawable and an office user sees it; a student who left three years ago can be anonymised and afterwards has no sensitive fields while the register row, admission number and enrolments remain; a used OTP, an expired preview and an abandoned session are gone after one sweep; the reason text of a pay change is not in `audit_events` and is readable and redactable through the audit screen; the roster query never selects medical notes for a basic reader; APAAR never appears unmasked without the sensitive read and every reveal is audited; the runtime login cannot DELETE a student; all suites pass.

### Task 13: Observability and incident readiness

Owner: one contracts owner, one migration owner, backend agents for `apps/api`, one frontend agent, one docs agent. Added on 18 September 2026 to close findings F6, F7, F8, F10, F11, F12 and F19 of the data protection assessment. Depends on Task 12. Stacked on the Task 12 branch.

**Decisions.**

- Reads of one person's record are audited; lists are not. The row is written by `protectedRoute` after the handler has answered, in its own short tenant transaction, so a read never fails because its audit failed to commit and a handler needs no change. Volume is bounded by the number of detail screens a person opens, not by roster paging.
- Every refusal a member receives is a `denied` audit row in that school's trail, written outside the transaction that refused, so the school's audit screen answers "who tried what". Anonymous refusals have no school and are counted in the access log only.
- The access log is a table in our own database, `access_log`, one row per `/api` request, kept 180 days and swept daily. Vercel Hobby offers no log drain and Sentry must never hold request data, so the database is the only sink under our control. The database is in Neon `us-east-1` today; CERT-In wants these logs kept in India, and Neon has no Indian region, so this is recorded as an open hosting decision for Task 14, not hidden.
- Account lockout is per identity and durable: ten failed password sign-ins lock the identity for fifteen minutes. A locked or disabled identity is answered exactly like a wrong password. Disabling an identity is an operator action through a script that uses the auth login, because no school owns a cross-school identity.
- A subject-access export is one permission-gated read that assembles everything the system holds about one student through the existing read plans, so the caller receives exactly the blocks they could already read one screen at a time, in one audited document.

**Schema (`packages/db/migrations/0010_observability.sql`, mirrored in `packages/db/src/schema.ts`).**

1. `access_log`: `id`, `at` timestamptz default now(), `method`, `route` (the route pattern, never the concrete URL), `status` integer, `code` text nullable, `user_id` uuid nullable, `membership_id` uuid nullable, `school_id` uuid nullable, `ip_hash` text nullable, `duration_ms` integer, `request_id` text. No foreign keys (a row must survive the deletion of what it names). Indexes on `(at)` and `(membership_id, at)`. No tenant RLS: a global infrastructure table like `auth_throttle`. `GRANT INSERT` to `erp_runtime` only; nobody else reads it through the API.
2. `auth_user`: add `disabled_at timestamptz`, `locked_until timestamptz`, `failed_sign_ins integer NOT NULL DEFAULT 0`. `erp_auth` already holds UPDATE.
3. Function `sweep_access_log()` owned by `erp_maintenance` (grant it `SELECT, DELETE` on `access_log`): delete rows older than 180 days, returning `(item, count)` like the Task 12 functions; EXECUTE to `erp_runtime` only. The maintenance route calls it.
   `packages/db/tests`: the runtime can INSERT but not SELECT or DELETE `access_log`; `erp_auth` cannot touch it; the sweep removes a 200-day-old row and keeps a 100-day-old one.

**Contracts.** New permission `students.export_subject` (student; school, own_children; privileged at school scope only). Owner and principal at school; parent at own_children; nobody else. `SubjectAccessExport` = `{ generatedAt, schoolId, student: StudentBasic, sensitive?: StudentSensitive-with-full-apaarId, medical?: StudentMedical, guardians: Array<GuardianPrivate | GuardianContact>, enrollments: EnrollmentSummary[], documents: DocumentSummary[], consents: ConsentRecord[], accessHistory: Array<{ at, action, actorDisplayName, outcome }> }`, strict; blocks present only when the caller holds the matching read permission on this student, and `accessHistory` only when the caller holds `audit.read` (it lists the audit rows whose `target_id` is the student). Endpoint `GET /api/schools/:schoolId/students/:studentId/subject-access`. Regenerate the matrix.

**API.**

- Read auditing (F6). `RouteDefinition` gains `auditRead?: { targetType: string; param: string; summary: string; detail?: (result) => Record<string, unknown> }`. When present and the handler succeeded, `protectedRoute` writes one `allowed` audit row (action = the route permission, target = the named param, `safeChanges` = `detail(result)` or `{}`) in a fresh `withTenantTransaction`; a failure there is logged and does not fail the response. Apply to: student detail (`detail` reports which blocks were present: `sensitive`, `medical`, `guardianContacts`), student guardians, student consents, staff detail (`detail` reports `employment`, `private`, `pay`), the APAAR reveal (replace its handler-written row with `auditRead` so there is exactly one), and the subject-access export. Document download already writes its own row inside its transaction and stays as it is.
- Denials (F7). `protectedRoute` catches an `ACCESS_DENIED` (from the gate or from the handler) and, when a request context exists, writes one `denied` audit row in a fresh tenant transaction: action = route permission, `targetType` = the permission's resource type, `targetId` null, `summary` "Refused: <permission> on <route pattern>.", `safeChanges { route, method }`; then rethrows. After writing it, count that membership's `denied` rows in the last ten minutes and, at twenty or more, call `reportDenialBurst({ schoolId, membershipId, count })`, which sends one Sentry event per membership per burst (fingerprint by membership id, level error, tags only ids). `reportDenial` gains the membership id tag when known.
- Access log (F7, F19). `apps/api/src/http/access-log.ts`: an `onResponse` hook for every `/api` request inserts one `access_log` row through the runtime pool: method, `request.routeOptions.url` (the pattern), status, our error code when one was sent (read it from a decoration the error handler sets), user, membership and school ids from `request.context` or `request.verified`, `ip_hash` = HMAC-SHA256 of the client address keyed by `AUTH_SECRET` (hex, first 32 characters), duration and request id. Fire-and-forget: the insert's promise is awaited by nothing the client waits for and a failure is logged once. Never the URL, query string, body, cookie or user agent. Skip `/api/health`.
- Lockout and disable (F10). In `auth/better-auth.ts`: a `before` hook on `/sign-in/email` and `/phone-number/verify` looks the identity up by the identifier and, when `disabled_at` is set or `locked_until` is in the future, returns the provider's own invalid-credentials error so a caller cannot tell a lock from a wrong password. An `after` hook on `/sign-in/email`: on an error result, `failed_sign_ins = failed_sign_ins + 1` for that email and `locked_until = now() + 15 minutes` once it reaches ten; on success, reset both. `resolveSession` reads `disabled_at` for the user on the auth pool and, when set, deletes the user's sessions and throws `AUTHENTICATION_REQUIRED`. Operator script `apps/api/scripts/disable-identity.ts` (`pnpm --filter @erp/api ops:identity -- --email <e> --disable|--enable|--unlock`) using `AUTH_DATABASE_URL`: sets or clears `disabled_at`, clears the lock, deletes sessions on disable, prints one line, never prints a secret.
- Credentials file (F11). `apps/api/scripts/dev-seed.ts` creates the output directory with mode `0o700` and writes the logins file with mode `0o600`.
- Subject access (F12). `modules/students/subject-access.ts`: the route above decides the student under `students.export_subject`, then assembles each block through the same reads and projections the detail routes use, deciding each block's permission on this record (`students.read_sensitive`, `read_medical`, `read_guardians` else `read_guardian_contact`, `read_enrollments`, `read_documents`, `read_consents`, and `audit.read` for the history). The sensitive block carries the full APAAR (opened from the ciphertext) because that is what a subject-access request is for; the export's own audit row (through `auditRead`) records that it happened. Anonymised students export the register fields only.
- Maintenance. The sweep route also runs `sweep_access_log()` on the runtime pool and reports its count under `access_log`.

Tests in `apps/api/tests`: opening a student detail leaves one `allowed` audit row naming the student with the blocks read; a refused detail read leaves one `denied` row visible to the owner on `GET /audit-events`; twenty refusals in a row trigger exactly one `reportDenialBurst` (inject a spy through the test server or assert on a module-level hook); every `/api` request writes one `access_log` row with the route pattern and no URL or query (assert with `adminPool`); the health route writes none; ten wrong passwords lock the identity, the eleventh attempt with the right password fails identically, a back-dated `locked_until` lets it in and resets the counter; a disabled identity's existing session is refused and removed; the subject-access export for a parent has the child's blocks the parent may read and no `accessHistory`, for the owner has `accessHistory`, and for a teacher is refused at the gate; the export writes exactly one audit row.

**Web.** The parent home gets "Download my child's record" per child and the student profile gets "Export this record" for `students.export_subject`; both fetch the export on click and save it as a JSON file named by admission number, never caching it. The audit log screen already shows `denied` outcomes; give the outcome a filter chip so a burst can be read. Nothing else.

**Docs.** New `docs/compliance/INCIDENT_RESPONSE.md`: severity levels; named roles (incident lead, communications, technical) with the current owner as all three until there is a team; the two clocks (CERT-In six hours from noticing; DPDP Board without delay and a full report within 72 hours; affected persons without delay) and what each notice contains; detection sources (Sentry denial burst, error reports, a parent's complaint, the access log); containment steps in order (disable the identity, suspend memberships, rotate `AUTH_SECRET` which ends every session, rotate provider keys, never rotate `DATA_ENCRYPTION_KEY` without the re-seal procedure); evidence collection (which `access_log` and `audit_events` queries, taken with the migrator login and recorded in the incident record as the one permitted direct read); communication templates; post-incident review. `RELEASE.md`: access log retention and region decision, the Sentry alert rule for the burst event, the operator script. `AUTHENTICATION.md`: lockout and disable. `PROTECTED_APIS.md` and `OPERATION_COVERAGE.md`: read auditing, denied rows, the export route. `WEB_SCREENS.md`. `DATA_PROTECTION.md`: F6, F7, F8, F10, F11, F12 closed with files; F19 marked decided-with-open-region and pointing at Task 14.
