...
| `GET /api/schools/:schoolId/invitations` | `members.invite` | Page request bounds, status filter (`pending` by default) | 200 | `INVALID_REQUEST`, `ACCESS_DENIED`, `AUTHENTICATION_REQUIRED` |
...
| `GET /api/schools/:schoolId/members/:membershipId/restrictions` | `access.manage` | Target in this school | 200 | `INVALID_REQUEST`, `ACCESS_DENIED`, `RESOURCE_NOT_FOUND` |
| `POST /api/schools/:schoolId/members/:membershipId/restrictions` | `access.manage` | `ROLE_RESTRICTION_RULES`, not self, not an owner, target active and holding a staff role, `expectedAccessVersion`, no restriction of that kind already in force, end date in the future | 201 | `INVALID_REQUEST`, `ACCESS_DENIED`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
| `POST /api/schools/:schoolId/members/:membershipId/restrictions/:restrictionId/lift` | `access.manage` | The same delegation, rule in force, `expectedVersion` of the rule | 200 | `INVALID_REQUEST`, `ACCESS_DENIED`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
...
`GET /members` takes four filters beside `page` and `pageSize`, all described by `MemberListRequest`: `status`, `role` (a role key), `staffId` (the membership a staff record belongs to) and `search`. They are one SQL clause with bound parameters, applied to the count and to the page alike, so the total is the filtered total and never the whole directory. `search` matches the name the directory shows, which means the staff record, the guardian record and, for a login with neither, `auth_user.name` looked up through the auth pool; a `%`, `_` or `\` in the text is escaped and looked for literally. The query is strict, so an unknown key or a status the enum does not hold is `INVALID_REQUEST`. None of this widens anything: `members.read` is still decided against the school before the handler runs, and a caller without it is refused whatever they filter on.

## The invitation lifecycle

An invitation row moves through `pending`, then one of `accepted`, `revoked` or `expired`. Creating one takes `members.invite` and `roles.assign`, and every proposed role must be in `assignableRolesFor` for the actor's roles. A partial unique index keeps one pending invitation per school and normalised identifier; a racing duplicate is answered `INVITATION_UNAVAILABLE`. Resending mints a new token, which silently kills the old one, and extends the expiry. Revoking closes the row. A pending row past its expiry is reported as `expired` in responses without a write, and is written as `expired` the moment somebody tries to use it. The list reads the same way: one SQL `CASE` decides the reported status, and both the status filter and the returned row use it, so an expired row leaves the pending list and appears under `expired` without anything being written. The list returns `InvitationSummary` rows built by the same mapper as create and resend, so it carries the masked destination and never the token or its digest, and being a read it writes no audit row.
...
Added in September 2026 because a school office asked to keep one person away from the numbers on a pupil's file without changing that person's role. An owner or principal (`access.manage`, school scope) takes away, for one member across the whole school, any of three keys: `students.read_sensitive` (Aadhaar, APAAR and the rest of the sensitive block), `students.read_guardians` (full guardian records, including PAN, Aadhaar and office address) and `students.read_medical`. The guardian contact card (`students.read_guardian_contact`) is not restrictable on its own: full guardian records carry the phone too, so hiding the card alone would promise more than it does, and staff who call families need it. The list is `RESTRICTABLE_PERMISSIONS` in `packages/contracts/src/access-rules.ts`.

A restriction is a `resource_access_rules` row with effect `deny` on the school target, a reason, a start and an optional end. Nothing new decides it: the policy already lets any applicable deny win, `planPredicate` already adds `AND NOT` for denies, and `capabilities` already drops a permission a school-wide deny removes. So a restricted person loses the field on the record, in lists, in export files, in profile PDFs and on the reveal routes at once. Migration `0020_member_restrictions.sql` lets the catalogue trigger accept these three keys on the school target, and only as a deny: an allow for them would widen what a role gives, and nothing may do that.

Adding or lifting one follows the write protocol above: lock the school, lock the target for an access change and compare its access version, apply the restriction delegation in `ROLE_RESTRICTION_RULES` (nobody restricts themselves or an owner; an owner restricts a principal, administrator, accountant or teacher; a principal restricts an administrator, accountant or teacher but not another principal; a parent role on the target is ignored). It is wider than suspension, where a principal manages teachers only, because a restriction can only take access away, bump the target's access version so their next request and any export job already queued are decided again, and write one audit row, `access.restrict` or `access.restriction_lift`, with the reason as the note. A member whose roles are only parent or student cannot be restricted: the control is for staff. Restrictions end with removal like every exception, and restore does not bring them back.
...
`invitations.test.ts` also covers the list: a new invitation appears in the default pending page and the response body contains neither the token nor the stored digest, a page and its total agree, an expired pending row leaves the pending list and appears under `status=expired` while the stored row stays `pending`, a revoked one appears only under `status=revoked`, a teacher is refused with `ACCESS_DENIED`, and a principal of the other school sees only their own school's rows.
...
`members.test.ts`: an owner reads the directory, the page size is honoured and an invalid one refused, a member of one school cannot read another school's directory, a parent has no permission to read it, an anonymous caller is told nothing, and an owner can explain another membership's access decision. Task 18 adds the filters: each of status, role, `staffId` and search narrows the page and the total together, a search finds somebody named only by their login as well as somebody named by a staff record, a wildcard character is looked for literally, an unknown query key is refused, and another school's staff id through this school's path finds nobody rather than leaking a row.
