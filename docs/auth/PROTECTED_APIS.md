...
Source files: [modules/index.ts](../../apps/api/src/modules/index.ts), [modules/shared](../../apps/api/src/modules/shared) (`route.ts`, `authorize.ts`, `audit.ts`, `version.ts`, `errors.ts`), then one folder per module: [attendance](../../apps/api/src/modules/attendance), [exams](../../apps/api/src/modules/exams), [report cards](../../apps/api/src/modules/report-cards), [fees](../../apps/api/src/modules/fees), [setup](../../apps/api/src/modules/setup), [students](../../apps/api/src/modules/students), [students-bulk](../../apps/api/src/modules/students-bulk), [staff](../../apps/api/src/modules/staff), [timetable](../../apps/api/src/modules/timetable), [dashboard](../../apps/api/src/modules/dashboard), [search](../../apps/api/src/modules/search), [audit](../../apps/api/src/modules/audit), [files](../../apps/api/src/modules/files). Their request and response contracts are the `module-*.ts` files in [`@erp/contracts`](./CONTRACTS.md).
...
Task 23. A pupil's own login; the behaviour is in [student login](#student-login). Every path is under `/api/schools/:schoolId`. Each route decides `students.manage_login` on the pupil (`decideResource` on the `student` resource), so a pupil in another school, or an unknown id, is `RESOURCE_NOT_FOUND`. A refusal a person can act on carries a reason: `student_login_not_eligible`, `student_login_no_guardian_phone`, `student_login_exists`, `student_login_missing`.

| Method and path | Permission | Extra checks | Success | Error codes |
|---|---|---|---|---|
| `GET /students/:studentId/login` | `students.manage_login` | the pupil's login as `StudentLoginView`: state, username (the admission number), the school code, the blocker when one stands in the way, the primary guardian's phone masked, whether the sent password is still unchanged, when it was issued and last used; never a password or the generated address | 200 | `RESOURCE_NOT_FOUND` |
| `POST /students/:studentId/login` | `students.manage_login` | state `none` or `ended` (`student_login_exists`); no blocker (`student_login_not_eligible`, `student_login_no_guardian_phone`); issues the login and texts the password after the commit | 201 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `POST /students/:studentId/login/reset-password` | `students.manage_login` | state `active` or `switched_off` (`student_login_missing`); a primary guardian phone (`student_login_no_guardian_phone`); a new password texted after the commit, every session of the pupil ends | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `POST /students/:studentId/login/switch-off` | `students.manage_login` | state `active` (`student_login_missing`); `expectedVersion` is the membership's; the reason is the audit note; every session ends | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
| `POST /students/:studentId/login/switch-on` | `students.manage_login` | state `switched_off` (`student_login_missing`); the pupil still on the roll in Class 9 to 12 (`student_login_not_eligible`); `expectedVersion` | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
| `POST /students/logins/issue` | `students.manage_login` | decided at school scope; issues a login to every eligible pupil who has none (or whose login ended), at most 500 a call, one at a time; answers counts only | 200 | — |

`POST /api/student-sign-in` is not a school route: it is the pupil's sign-in door, public like the provider's sign-in routes. See [student login](#student-login).
...
| Method and path | Permission | Extra checks | Success | Error codes |
|---|---|---|---|---|
| `GET /exams?academicYearId=` | `exams.read` | the exam rows under the plan's `exam` face: only a school scope selects them, so a teacher or a parent gets an empty list; section counts in SQL | 200 | `INVALID_REQUEST` |
| `POST /exams` | `exams.manage` | school locked; the year in this school; first day and re-check deadline inside the year (`exam_dates_outside_year`); one exam per kind and year; the papers made from the year's sections and `grade_subjects` | 201 | `INVALID_REQUEST` |
| `PUT /exams/:examId` | `exams.manage` | `expectedVersion`; refused once any section is published (`exam_already_published`); the papers brought up to date | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
| `GET /exams/:examId` | `exams.read` | the exam decided again (the office's); per section the roster, each paper's entered and expected cells, the newest publication and whether a mark changed since, and why it is not ready to publish | 200 | `RESOURCE_NOT_FOUND` |
| `GET /exams/papers?academicYearId=&examId=&sectionId=&subjectId=` | `exams.read` | papers under the plan's `paper` face (a subject teacher's own subject in their own section, a class teacher's whole class, the office's school); each with its `window` | 200 | `INVALID_REQUEST` |
| `GET /exams/papers/:paperId` | `exams.read` | the paper decided again; the roster enrolled in the section on the exam's first day, names through `students.read_basic`; the current mark of every filled cell | 200 | `RESOURCE_NOT_FOUND` |
| `PUT /exams/papers/:paperId/marks` | `exams.record_marks` | school locked; the paper decided again; from the first day to the end of the re-check deadline (`exam_not_started`, `exam_recheck_deadline_passed`); every line on the roster, in the exam and within the maximum (`exam_pupil_not_on_roster`, `exam_component_not_in_exam`, `exam_mark_above_maximum`); a change to a saved cell needs `change` (`exam_change_needs_reason`); a new row only where the value differs; the reason is the audit note | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `POST /exams/papers/:paperId/corrections` | `exams.manage` | the same from the first day on, before or after the deadline and after publishing; always a reason kind and a reason; rows of kind `correction` | 201 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `GET /exams/papers/:paperId/students/:studentId/history?component=` | `exams.read` | every row of one cell, oldest first, under the mark plan; who recorded it only through `staff.read_directory` | 200 | `RESOURCE_NOT_FOUND` |
| `POST /exams/:examId/sections/:sectionId/publish` | `exams.publish` | school locked; after the deadline (`exam_publish_before_deadline`); every roster pupil has a mark or a status for every paper and component (`exam_section_incomplete`); again only after a change (`exam_nothing_to_publish`); one publication row appended | 201 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `GET /exams/students/:studentId/results?academicYearId=` | `exams.read` | the pupil decided again; live marks for a staff reader, marks as at the newest publication for a family reader, and grades only when the school shows grades; audited through `auditRead` | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `POST /exams/papers/:paperId/export` | `exams.export` | the paper decided again under this key; the producer re-reads the sheet under the requester's plans | 202 | `RESOURCE_NOT_FOUND` |

### Report cards and the school logo

| Method and path | Permission | Extra checks | Success | Error codes |
|---|---|---|---|---|
| `GET /report-cards/settings` | `exams.read` | the school's grade bands, results display and layout, or the defaults with version 0 | 200 | — |
| `PUT /report-cards/settings` | `exams.manage` | school locked; decided at school scope; `expectedVersion` (0 for the first save); bands checked by `gradeBandsProblem` (`grade_bands_overlap`, `grade_bands_gap`, `grade_bands_out_of_range`, `grade_bands_duplicate_label`) | 200 | `INVALID_REQUEST`, `VERSION_CONFLICT` |
| `GET /report-cards/sections/:sectionId/terms/:term/entries` | `report_cards.read` | the section decided again (the class-teacher post or the office); the term's roster with each pupil's co-scholastic grades and remarks | 200 | `RESOURCE_NOT_FOUND` |
| `PUT /report-cards/sections/:sectionId/terms/:term/entries` | `report_cards.manage` | school locked; every pupil on the term's roster (`report_card_pupil_not_on_roster`); `expectedVersion` per row, 0 for a new one; remarks go nowhere but the row | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND`, `VERSION_CONFLICT` |
| `GET /report-cards/sections?academicYearId=&card=` | `report_cards.read` | sections under the plan's `roster` face; counts of pupils, grades entered, cards published and cards changed since | 200 | `INVALID_REQUEST` |
| `GET /report-cards/sections/:sectionId/cards/:card` | `report_cards.read` | the section decided again; the exams the card needs and where each stands; each pupil's newest version and whether it would change | 200 | `RESOURCE_NOT_FOUND` |
| `POST /report-cards/sections/:sectionId/cards/:card/publish` | `report_cards.publish` | school locked; every exam of the card published with no change since (`report_card_exams_not_published`, `report_card_exams_changed`); named pupils on the roster; unchanged cards skipped (`report_card_nothing_to_publish` when all are); one new version per changed card | 201 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `GET /report-cards/students/:studentId?academicYearId=` | `report_cards.read` | the pupil decided again; every published version of that year under the card plan | 200 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `GET /report-cards/versions/:versionId` | `report_cards.read` | the card decided again; grades only in a family view of a grades card; audited through `auditRead` | 200 | `RESOURCE_NOT_FOUND` |
| `POST /report-cards/versions/:versionId/export` | `report_cards.export` | the card decided again; a single-record kind, so the download decides it again | 202 | `RESOURCE_NOT_FOUND` |
| `POST /report-cards/sections/:sectionId/cards/:card/export` | `report_cards.export` | the section decided again; the newest version of every roster pupil | 202 | `INVALID_REQUEST`, `RESOURCE_NOT_FOUND` |
| `PUT /school/logo?expectedVersion=` | `school.update` | registered by hand like a photograph: PNG or JPEG up to 512 KB, cleaned before it is stored; the school profile's version | 204 | `INVALID_REQUEST`, `VERSION_CONFLICT`, 413 |
...
| `GET /exports/:jobId` | one of `students.export`, `staff.export`, `audit.export`, `timetable.read`, `fees.export`, `fees.read`, `attendance.export`, `attendance.read`, `staff_attendance.export`, `exams.export`, `report_cards.export` | the job's own recorded permission re-decided, plus freshness | 200 | `RESOURCE_NOT_FOUND` |
...
Fifteen kinds of job exist. `students`, `staff`, `audit` and `exam_marks_register` produce a spreadsheet; `student_profile`, `staff_profile`, `fee_receipt`, `attendance_pupil_month`, `report_card` and `report_cards_section` produce a document; `timetable`, `fee_dues`, `fee_collections`, `attendance_register` and `staff_attendance_register` produce whichever the request asked for. The fees, attendance and exams sections below say what each module's kinds hold. The producers live in [`apps/api/src/exports`](../../apps/api/src/exports) and each one is registered by kind, so a route never names a file format and the runner never names a kind.
...
Task 21. Every class of every school follows the CBSE two-term scheme, which is a constant in `@erp/contracts`: term 1 is periodic test 1 (out of 10), notebook (5), subject enrichment (5) and the half-yearly exam (80); term 2 the same with periodic test 2 and the annual exam. The notebook and subject enrichment marks are recorded against the term's main exam, so a year has four exams. A school sets only when each exam happens and when its re-check window closes. Source: [modules/exams](../../apps/api/src/modules/exams) (`common.ts` for everything both modules share) and [modules/report-cards](../../apps/api/src/modules/report-cards).

**Papers and the roster.** A paper is one exam, one section and one subject. Saving an exam makes a paper for every section of that year and every subject its class has in `grade_subjects`, and removes a paper that no longer belongs and has no mark. A subject whose type is `co_scholastic` (physical education, art) gets no paper: on the CBSE card it is graded A, B or C through the four fixed co-scholastic areas by the class teacher, never out of 80. A paper's roster is the pupils enrolled in the section on the exam's first day. There is no record of who takes an optional subject, so a pupil who does not is marked exempt.

**A mark.** A number with at most one decimal place, never above the component's maximum, stored as whole tenths; or a status: absent (counts as zero), medical leave or exempt (the component is left out). `scoreParts` in `@erp/contracts` is the one scoring rule: the counted components' marks over what they are out of, scaled to 100, one decimal, half up; a component with nothing entered is left out too. A term is the two exams of the term together; the final figure of a subject is the mean of its two terms; the overall result is the mean of the subjects and `pass` when every subject reached 33. A grade is the school's band holding the figure rounded half up to a whole number.

**The marks sheet is one write.** `PUT /exams/papers/:paperId/marks` takes every filled cell; a cell left out is not entered yet and a filled cell is never emptied. It writes a new row only where the value differs. The subject teacher may save from the exam's first day to the end of the re-check deadline in the school's timezone; nothing runs at the deadline, the state is worked out from the date each time. Every change to a cell already saved needs a reason kind (re-check, entry error, other) and a reason, whoever saves: the kind is on the row and in `safe_changes`, the words are the audit note.

**Nothing is edited.** `exam_marks` and `exam_publications` take INSERT and nothing else, by grant and by trigger. A change is a new row with the next revision pointing at the row it supersedes; the current mark is the highest revision. After the deadline only the office changes a mark, with `POST .../corrections` under `exams.manage`, always with a reason, as a row of kind `correction`. The original row stays.

**Publishing.** The office publishes one exam for one section under `exams.publish`, only after the deadline and only when every pupil on the roster has a mark or a status for every subject and component. A publication is a row; publishing again after a correction appends another. A correction after publishing leaves the section published and marks it "changed since published".

**Who reads what.** Every read ANDs the `exam` plan over the face it reads (`examScopedTable('exam' | 'paper' | 'mark' | 'pupil')`) and the `students.read_basic` plan for names. A subject teacher reaches their own subject in their own section (`assigned_subjects`, the whole triple). A class teacher reaches every subject of their own class (`assigned_sections`, which for exams and report cards is the class-teacher post alone, so a teaching assignment never opens another subject). The office reaches the school. A parent reaches their own child's marks for every year, and only published ones: the scope term matches a mark only when a publication of its exam for its section is at least as new as the mark, so a parent sees each mark as it stood at the newest publication and a correction stays invisible until the office publishes again. Which way a reader reaches the pupil decides the shape of the answer: `planPredicateWithout` tells a staff reader (live marks, always as marks) from a family reader (published marks, and grades only when the school has chosen grades), and the marks are removed on the server. No class average is shown anywhere a parent looks.

**Report cards.** A term 1 card needs periodic test 1 and the half-yearly published for the section, a final card all four exams, each with no change since its publication (otherwise `report_card_exams_not_published` or `report_card_exams_changed`), so a card never shows a figure the family's results page does not. The card's roster is the section on the first day of its last main exam. Publishing freezes the content: the figures of every subject and term, the co-scholastic grades, the attendance of each term, the grade bands, the marks-or-grades choice, the layout and the school's header lines at that moment. A later change to any of them does not touch a published version; the section screen says which cards would change, and publishing again writes a new version (only for the cards that changed) and keeps the earlier one. A card's attendance is `attendanceFiguresCte` over the term's window (term 1 from the year's first day to the last day of the half-yearly, term 2 from the next day to the last day of the annual): working days are the school days while enrolled less leave days (leave neither helps nor hurts, the Task 20 rule), days present count present and late whole and half days as half.

**The class teacher's part.** The co-scholastic grades (A, B or C for work education, art education, health and physical education, and discipline) and the remarks, per pupil and term, saved as one write of the term's roster under `report_cards.manage`. Remarks are free text about a child: they live in the row and on the published card and nowhere else, not in `safe_changes` and not in an audit note, so anonymising the pupil really clears them.

**Settings.** One row per school holds the grade bands (the CBSE 8-point scale until the school saves its own; contiguous, no overlaps, from 0 to 100, checked by `gradeBandsProblem`), the marks-or-grades choice, and the layout: logo or not, which header lines, which blocks in which order, up to three signature labels and a footer note. The layout is structured settings, never HTML or CSS. Saving is `exams.manage` at school scope.

**The logo** is bytes in the private store exactly as a photograph is, PNG or JPEG only (the PDF library cannot draw WebP), up to 512 KB, cleaned before it is stored, under `school.update`. It is served under `holidays.read`, the every-role floor, so a teacher and a parent see it on a card. A published card records whether the logo was shown, not the picture: a card printed later carries the school's current logo.

**Audit.** A save leaves one row under `exams.record_marks` naming the paper and counting first entries and changes; a correction one under `exams.manage` with the reason as its note; a publication one under `exams.publish`; a co-scholastic save one under `report_cards.manage` with counts only; a card publication one under `report_cards.publish`. Reading one pupil's results or one card leaves an `auditRead` row; lists leave none. Nothing here is in `FINANCE_AUDIT_ACTIONS`.

**Files.** Three job kinds, one producer each: `exam_marks_register` (a paper's marks as Excel, under `exams.export`), `report_card` (one published card as a PDF, under `report_cards.export`, which a parent holds for their own child, so the parent's copy needs no read key) and `report_cards_section` (the newest card of every pupil in a section as one PDF). All three name one record, so the download decides it again. A card is always printed as it was published, in its own display mode, because it is the document handed to the family.

**Anonymising a pupil** clears the remarks in `report_card_entries` and on every published version (the database allows exactly that change to a card). Marks, grades and published figures stay: they are the pupil's academic record, kept permanently like the admission register. See [the data protection assessment](../compliance/DATA_PROTECTION.md) section 13.
...
**Pupils and ranges of classes (Task 23).** Every audience made of pupils (`school`, `grade`, `grade_range`, `section`, `pupil`) carries `recipients`: `families` (the default, everything above), `students`, or `both`; the staff audiences carry none (`messages.recipients` is null). `grade_range` is every class from `fromGradeId` to `toGradeId` in the school's class order (`grades.sort_order`, both ends included; the first must not come after the last, otherwise `INVALID_REQUEST`), the pupils enrolled this year in their sections; it is the office's, decided like `grade` (at school scope), stored as `grade_id` and `grade_to_id`, and labelled "Class 6 to Class 8". For `students` and `both`, one more row per pupil in the audience: `is_student`, `student_id` the pupil, `membership_id` the pupil's own membership when their login is `active` (then `in_app` and `delivered`), otherwise no membership and `no_contact`; `email_status` is always `none`. The pupil rows need no consent: the notice is to the pupil in the school's own app, and nothing about the pupil leaves it. A pupil reads their rows through `self`, as every member reads their inbox, so a notice to the families only is never in a pupil's inbox and a notice to the pupils only is never in a guardian's. Counts and the preview report `pupils` (and `pupilsInApp`) among the recipients; a delivery list names a pupil's row by the pupil's name with the relation "Pupil". Automatic messages are written with `recipients = 'families'`, except `birthday_pupil`, which is `both`. Labels: a pupil audience reads "Family of Aarav Sharma", "Aarav Sharma" or "Aarav Sharma and family"; the recipients of any other audience are shown next to its label by the screen.
...
**Who gets one.** A pupil is eligible when their status is `active`, they are not anonymised, and they have an open enrolment (`left_on IS NULL`) in a section of a year that is not closed whose class is numbered 9 to 12 (`grades.level`, `STUDENT_LOGIN_LEVELS`). The class number is set in setup; migration 0019 filled it in from names such as "Class 9", "Std 10" or "Class XI". Nursery, LKG and UKG have none. The password goes to the pupil's primary guardian (`student_guardians.is_primary`, not anonymised) at their `phone`; a pupil whose primary guardian has no phone number cannot be given a login yet (`no_guardian_phone`).

**The login.** One identity per pupil: an `auth_user` with a generated address `<uuid>@student.invalid` (never sent anything, never returned by any route), the pupil's name, a credential account, and `must_change_password` set. One membership: kind `student`, the `student` role and nothing else, linked to the pupil through `membership_student_links` (one per pupil per school). Its state is read from the membership: `none` (no link), `active` (status `active`), `switched_off` (`suspended`), `ended` (`removed`).

**When it is made.** Automatically, after the commit of an admission, a bulk import or a promotion, for every pupil of that write who is now eligible and has no login (or whose login ended): the admission, import and promotion routes call `issueStudentLogins(deps, context, studentIds, trigger)` once their own transaction has committed. That call never fails the request that triggered it: a pupil it could not serve is counted and logged (counts only) and the office sees why on the pupil's page. The office issues one from the pupil's page, and every missing one at once with `POST /students/logins/issue`; that is also how the pupils already in Class 9 to 12 when this release arrives get theirs. Each issue is its own pair of steps: the identity is created through the auth pool (`provisionStudentIdentity`), then one tenant transaction locks the school, checks the state and eligibility again, writes the membership, its role and the link (or, for an ended login, points the existing membership at the new identity and sets it `active`, bumping `access_version`), queues a `delivery_outbox` row (`event_type = 'student_password'`, masked phone, no secret) and writes one audit row; when that transaction does not commit the new identity is removed again (`removeUnusedStudentIdentity`). After the commit the password is texted (`purpose: 'student_password'`) and the outbox row marked `sent` or `failed`. The password exists only in memory between generation and the text: never in a table, a log, an audit row or a response.

**The text.** "`<school code>`: sign-in for admission no. `<admission number>`. Password: `<password>`. Choose a new one at first sign-in." Text messages are held until Task 16: in a test build the text is in the held-message outbox (`GET /api/held-codes`) or the development outbox, like a parent's code.

**Signing in.** `POST /api/student-sign-in` with `{ schoolCode, admissionNumber, password, sharedDevice? }`. The identity pool's `student_sign_in_user(code, admission number)` finds the identity only while the pupil is on the roll and the login is on, comparing both without regard to case or surrounding space. The route then signs in through the provider's email door with the generated address, inside `studentSignInScope`, so the password check, the lockout after ten failures and the session cookie are exactly the provider's. An unknown school, admission number or pupil goes through the same door with an address nobody holds, so every failure is the same `AUTHENTICATION_REQUIRED` with the same timing. The email door refuses a `@student.invalid` address typed into it. Phone codes and email resets never reach a pupil's identity. The answer is `{ signedIn: true, passwordChangeRequired }` and the session cookie.

**A class signs in together (September 2026).** The provider's own limit is three sign-ins every ten seconds per address and path, and a computer lab shares one public address, so from the fourth pupil on everyone waited. The provider's limit is skipped for this route only (`customRules['/sign-in/email']` answers `false` while `studentSignInScope` is set) and replaced by two durable buckets in `auth_throttle`, taken before the provider is called and taken the same way for an unknown school or admission number: five attempts a minute for one pupil account (keyed on a SHA-256 of the school code and admission number, never the text itself) and 150 a minute for one address (keyed on a SHA-256 of the address). Either answers `RATE_LIMITED` with `retry-after`. The lockout after ten failures is unchanged. Staff sign-in keeps the provider's three in ten seconds.

**First sign-in.** While `must_change_password` is set, `/api/me` carries `session.passwordChangeRequired: true` and every school route answers `PASSWORD_CHANGE_REQUIRED` (403); the provider's `change-password` route stays open and clears the flag when it succeeds (other sessions end as for everyone). A whole class changes its passwords at once, so the provider's per-address limit on `change-password` is widened to 120 a minute and the budget that matters is per person: five failed attempts in fifteen minutes answer `RATE_LIMITED`, counted in `auth_throttle` against the signed-in user and cleared by a success. A pupil changes their password again whenever they like from account security.

**Sessions.** A pupil's session lives twelve hours and ends after an hour idle, like a teacher's; shared-device mode shortens it as for everyone. No second factor. A pupil whose login is switched off or ended (`student_login_state = 'inactive'`) is refused a new session and loses every live one on their next request.

**What a pupil reads.** The `student` role grants, all at `own_record` unless named: `grades.read`, `sections.read`, `subjects.read`, `holidays.read` (school), `students.read_basic`, `students.read_enrollments`, `timetable.read`, `dashboard.read`, `attendance.read`, `exams.read`, `report_cards.read`, and `communication.read` at `self`. `own_record` is written exactly like `own_children` with the pupil themself as the only child (`RelationshipFacts.ownStudentId` from `membership_student_links`): results and report cards answer published rows only, a class, section, subject or timetable row answers through the pupil's current enrolment, and a message never answers through the pupil it is about. A pupil holds no fee, guardian, consent, document, medical, sensitive, export, staff or setup-write key, and no route needs special-casing for that: the gate refuses them. The student home is the dashboard audience `student`: the pupil's own card (class, class teacher, today's lessons, this month's attendance, the newest published report card) and the holidays ahead.

**Switching off and ending.** Only the office switches a login off (`suspended`, sessions end) and on again. A guardian cannot. A login ends (`removed`) inside the same transaction when the pupil leaves (`POST /students/:studentId/leave`) or is anonymised, through `endStudentLogin(conn, context, studentId)`; the existing audit row of that write gains `studentLoginEnded: true` rather than a second row, and the sessions end after the commit. Finishing Class 12 is leaving. The ended identity has no membership left, so the credential sweep removes it after 30 days as for any adult.

**Messages to pupils.** A notice to a pupil audience (the school, a grade, a range of classes, a section, one pupil) goes to `families` (the default), `students` or `both` (`messages.recipients`). A pupil's own row (`is_student`) is written for every pupil in the audience: `delivered` and in the app when their login is on, `no_contact` otherwise; a pupil never gets an email and no consent row gates a notice in the pupil's own app. The automatic messages about absence, results, report cards and fees go to families only; a birthday wish goes to both. See [communication](#communication).

**Audit.** Every write leaves one `students.manage_login` row on the pupil (`targetType: 'student'`) with `safeChanges.change` one of `issued`, `password_reset`, `switched_off`, `switched_on`, and `trigger` (`office`, `admission`, `import`, `promotion`, `bulk`) for an issue; the switch-off reason is the note. A pupil signing in leaves the access log row every request leaves.
...
- `export_jobs.status` has a check constraint for the four contract values, and `kind` for the fifteen job kinds; a producer writing anything else would answer `SERVICE_UNAVAILABLE`.
...
Task 21 adds `modules-exams.test.ts` (14), `modules-report-cards.test.ts` (11) and `modules-exams-exports.test.ts` (4) to `apps/api/tests`, exam cases to `subject-access.test.ts` (a parent's export carries published marks only; the owner's carries live marks), `modules-lifecycle.test.ts` (anonymising clears the remarks in both tables and leaves the grades, the content and the hash) and `modules-dashboard.test.ts` (the three exam blocks), `packages/contracts/tests/exams.test.mjs` (11: the scoring rule, the final and overall figures, `gradeFor`, every `gradeBandsProblem` reason and the defaults), six exam and report card plan cases in `packages/authz/tests/scope.test.ts` and four in `policy.test.ts`, `packages/db/tests/exams-immutable.test.mjs` (6: the runtime login cannot UPDATE or DELETE a mark or a publication, two marks at one revision and a second revision without a reason kind are refused, a mark above its component's maximum is refused, a mark cannot point at another section's paper, a published card allows its remarks cleared and nothing else, and RLS hides another school's rows) and `tests/security/exams.test.ts` (11). The module files prove the marks sheet (revision 1 with one audit row and no note; a change without a reason refused with nothing written; a change with a reason writing revision 2 and leaving revision 1, the reason in the note only; a pupil off the roster, a component outside the exam and a mark above the maximum refused with nothing written; before the first day and after the deadline refused with their reasons), the office correction (after the deadline, kind `correction`, the reason in the note), publishing (before the deadline and while incomplete refused, a second publication refused until a mark changes), a parent's results (nothing before publishing, the published figures after, a correction invisible until published again, grades alone under grades), the settings (every band refusal, a first save and a stale version), the class teacher's entries (no remarks text in the audit row), card publishing (refused before the exams are published and after a change since, versions frozen through a band, a layout and a mark change, republishing keeping version 1) and the logo (PNG accepted, WebP refused, oversize 413). The security file asks the adversarial half: school B's pupil, section, subject, exam, paper, mark and card ids answering exactly like invented ids on twenty-one route shapes; a subject teacher on another subject of their section, on their subject in another section and after their assignment ended; a class teacher recording marks for a subject they do not teach; a parent and another family's child in both years after promotion; the accountant refused on every route with one denied row each; a parent before and after publishing; list and detail agreeing; and a published card that stays the same after the bands, the layout and a mark change. On a fresh `erp_test` the counts are: contracts 48, db 47, authz 74, api 539, web 376, security 113.
