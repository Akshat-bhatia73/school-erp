...
| `finance` | Always at row level. The narrowing is done by field groups, not by rows. |
...
`fieldGroupsFor(permission)` is a fixed map, exported and used by both the decision and the caller. Reads of student data return `student_basic`, `student_sensitive`, `student_medical`, `guardian_contact`, `guardian_private` or `document_metadata`; staff reads return `staff_directory`, `staff_employment`, `staff_private` or `staff_pay`; `timetable.read` returns `timetable`, `audit.read` returns `audit_summary`, and the setup reads (school, academic years, grades, sections, subjects, holidays) return `setup`. Every write permission returns an empty list. A successful decision carries the groups the caller may project; Task 5 does the projection and must not widen it.
...
Ownership transfer and role escalation are Task 4, and they need more than a version check: `isFreshMfa` in [apps/api/src/auth/assurance.ts](../../apps/api/src/auth/assurance.ts) is the shared five minute freshness check that already guards `two-factor/disable`, and it is the check those flows must reuse before `mayTransferOwnership` is consulted. There is no code for the transfer itself yet.

## Delegation rules

`src/delegation.ts` is pure and the service does not call it. Task 4 checks it in addition to the `roles.assign` and `members.*` permissions.
...
- Ownership transfer, invitations and the role assignment endpoints are not written. The delegation rules and `isFreshMfa` are in place for them.
