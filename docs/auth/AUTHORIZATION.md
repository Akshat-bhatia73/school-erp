...
1. The permission must be a known `PermissionKey` and its catalogue availability must be `active`. A reserved permission such as an assistant key denies with `ACCESS_DENIED`.
2. A student membership must carry the `student` role and nothing else, and an adult membership must not carry it; anything else denies with `ACCESS_DENIED` (`studentKindConsistent`). The database refuses such a membership too.
...
| `self` | The resource carries a `staffId` equal to the caller's own staff record. A `staff_attendance` row answers through its staff member. For `communication` it is by membership instead, for every kind of member: a message answers when the caller wrote it or, while it is sent, when it shows in the caller's inbox; a recipient row when the caller wrote the message or, while it is sent, when the row is the caller's own in-app copy. |
| `assigned_sections` | The caller has a teaching assignment effective now whose section and academic year both match the resource, or is the class teacher of that section in a year that is not closed. The class teacher post names no subject, so it never counts for `assigned_subjects`. An `attendance` mark and a section's register answer through their section and year; a pupil's calendar answers through the pupil's current enrolment, as a student does. For `exam` and `report_card` the scope is the class-teacher post alone: a teaching assignment never counts, so a maths teacher who teaches 6A does not read 6A's science marks or its report cards. |
| `assigned_subjects` | The same assignment also matches one of the resource's subjects. An exam paper and a mark carry their section, year and subject and must match the triple as a whole; the pupil whose results are opened is reached when the caller teaches any subject in the pupil's current section, and the marks shown are still narrowed subject by subject. A report card is every subject on one page and is never reached this way. |
| `own_children` | The resource's `studentId` is one of the caller's approved children. A `fee` row answers through the pupil it belongs to; a fee head or a fee structure belongs to the school and names no pupil, so it never matches. An `attendance` mark and a pupil's calendar answer through the pupil, for every year the child was here; a section's register is a shared row and answers through a child currently in it. An exam mark answers through its pupil, for every year, and only once it is published: a publication of its exam for its section at least as new as the mark, so a parent sees each mark as it stood at the newest publication. A published report card version answers through its pupil; the class teacher's working entry, an exam's own row and a paper never do. |
| `own_record` | A pupil's own login (Task 23). Written exactly like `own_children` with the pupil themself as the only child: `RelationshipFacts.ownStudentId` comes from `membership_student_links`. Results and report cards answer published rows only; a class, section, subject or timetable row answers through the pupil's current enrolment; a message never answers through the pupil it is about (a pupil's inbox is `self`). |
...
Relationship facts are loaded once per request in `src/snapshot.ts`: the staff link, the teaching assignments effective at `context.now`, and the approved children. `loadResourceFacts` maps each resource type to the attributes the predicates need, and returns null for reserved types such as the assistant. The facts carry the caller's membership id, which the `communication` self scope needs.
...
`exam` has four faces: `examScopedTable('exam' | 'paper' | 'mark' | 'pupil')`, all taking one `exam` plan. `exam` is the exam's own row (dates and deadline), which only a school scope reaches; `paper` is one exam, section and subject, the sheet a subject teacher fills in; `mark` is one stored mark, carrying a `published` predicate; `pupil` is the child whose results are being read. `report_card` has four as well: `reportCardScopedTable('card' | 'entry' | 'roster' | 'pupil')` for a published version, the class teacher's working entry, the section whose cards are prepared and the pupil whose cards are listed. `ScopedTable.published` is the SQL boolean a family scope needs for these two types, and `ResourceFacts.published` is the same answer for a single decision, loaded by `loadResourceFacts` with the same `EXISTS`. `planPredicateWithout(plan, table, scopes)` builds the predicate with some scopes left out; it can only select fewer rows, and the exams module uses it to tell a staff reader (live marks) from a family reader (published marks, and grades alone when the school says so). Exceptions do not reach exams or report cards: no key of either is in `RESOURCE_RULE_PERMISSIONS`.
...
`communication` has two list faces, `communicationScopedTable('message' | 'recipient')`, both taking one `communication` plan. Both carry the section and year of a section message or a message about one pupil, which is how `assigned_sections` reaches them (class-teacher post and teaching assignments alike); a message to the school, a grade or the staff names no section and is reached only at school scope or by `self`. `own_children` and `own_record` never match a message: a family reads what was addressed to it, so consent decided per guardian cannot be bypassed through the child. `loadResourceFacts` looks a `communication` id up as a message, a recipient row, a section, a pupil, the school, a grade, a staff member and a template, so a send can be decided on its target before any message exists: a section and a pupil carry section facts, the others none, which leaves them to the school scope alone.
