...
| `own_record` | Never. Student login is disabled. For `exam` and `report_card` the term is written like `own_children`, published rows only, so turning student login on cannot open an unpublished mark. |
