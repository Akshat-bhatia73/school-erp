...
1. The permission must be a known `PermissionKey` and its catalogue availability must be `active`. A reserved permission such as a communication key denies with `ACCESS_DENIED`.
...
Relationship facts are loaded once per request in `src/snapshot.ts`: the staff link, the teaching assignments effective at `context.now`, and the approved children. `loadResourceFacts` maps each resource type to the attributes the predicates need, and returns null for reserved types such as communication.
