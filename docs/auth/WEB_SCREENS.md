...
## Rough edges, September 2026

**Member restrictions.** The member sheet (Settings → Users) has a "Restrictions" tab for a viewer holding `access.manage`, on anyone but themselves; the list loads when the tab opens. Each restriction in force shows what it hides ("Sensitive details (Aadhaar, APAAR)", "Full guardian records (PAN, Aadhaar, office address)", "Medical information"), "Until 31 Mar 2027" or "Until lifted", who added it and the reason. "Add restriction" and "Lift" render only when the list's `allowedActions` holds `access.manage`; both ask for a reason, adding also takes an optional end date (end of that day in India time) and sends the member's `accessVersion`. After a change the members prefix is invalidated and the sheet takes the refetched member, so its next action does not meet a stale version. The audit log names the two events "Restricted a member" and "Lifted a restriction".

**Allowing school messages.** The parent home shows "Get messages from the school" at the top when the dashboard carries the viewer's `guardianId`, the viewer holds `students.manage_consents` and at least one child is waiting on `communication`. "Allow school messages" records the consent for each waiting child, one child at a time, then refreshes the home and each child's consents. Nothing is recorded on page load. The waiting list counts only this guardian's newest row per purpose, the rule the message sender uses.

**Photograph at admission.** The pupil step has an optional photograph (JPEG, PNG or WebP, shrunk to under 1 MB when chosen, with a preview). It lives outside the draft. The review step warns when no guardian ticked consent for photographs, and then the picture is not sent. After the admission is saved the photograph is uploaded; if that fails the pupil stays admitted, an error toast says to add it from the pupil's page, and the screen goes to the pupil.

**Import columns.** The import template, help and column mapping gain "Student Aadhaar", "Guardian Aadhaar", "Guardian PAN" and "Guardian Office Address", all optional and matched without regard to case. The screen checks every row against the contract before sending, so a bad number shows its row, column and a plain sentence; a 12 digit Aadhaar that Excel stored as a number is read back as its digits.
